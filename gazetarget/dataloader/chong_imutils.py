...
def get_head_box_channel(x_min, y_min, x_max, y_max, width, height, resolution, coordconv=False):
    ...
    # for i in range(len(otherheads)):
    #     x_min, y_min, x_max, y_max  = otherheads[i]
    ...
    if coordconv:
        ...
        # cv2.imshow("headchannel", head_channel)
        # cv2.waitKey(0)
...
def get_object_box_channel(otherheads, width, height, resolution, coordconv=False):

    object_channel = np.zeros((resolution,resolution), dtype=np.float32)
    for i in range(len(otherheads)):
        x_min, y_min, x_max, y_max  = otherheads[i]
        ...
        if coordconv:
            ...
            object_channel = []
            for i in unit:
                object_channel.append([unit+i])
            object_channel = np.squeeze(np.array(object_channel)) / float(np.max(object_channel))
            object_channel[head_box[1]:head_box[3],head_box[0]:head_box[2]] = 0
        else:
            object_channel[head_box[1]:head_box[3],head_box[0]:head_box[2]] = 1

            cv2.imshow("objectchannel", object_channel)
            # cv2.waitKey(0)
