...
import math
from gazetarget.lib.pytorch_convolutional_rnn import convolutional_rnn
import numpy as np
...
class ModelSpatial(nn.Module):
    ...
    def __init__(self, block = Bottleneck, layers_scene = [3, 4, 6, 3, 2], layers_face = [3, 4, 6, 3, 2]):
        ...
        # scene pathway
        # self.avgpool_scene = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)#nn.AvgPool2d((2, 2))
        ...
        # face pathway
        # self.avgpool_face = nn.Conv2d(3, 64, kernel_size = 7, stride = 2, padding = 3, bias = False)#nn.AvgPool2d((2, 2))
        ...
        self.attn2 = nn.Linear(784, 1*7*7)
    ...
    # # def forward(self, images, head, face):#, objects):
    # def forward(self, images, head, face, objects):
    #     # face = self.avgpool_face(face)
    ...
    #     #we can reduce the scene binary image as well 
    #     object_reduced = self.maxpool(self.maxpool(self.maxpool(objects))).view(-1, 784)
    #     object_attn_weights = self.attn2(object_reduced)
    #     object_attn_weights = object_attn_weights.view(-1, 1, 49)
    #     object_attn_weights = F.softmax(object_attn_weights, dim=2) # soft attention weights single-channel
    #     object_attn_weights = object_attn_weights.view(-1, 1, 7, 7)
    ...
    #     # print("scene_feat !!!!!!!!!", scene_feat.shape)
    ...
    #     attn_applied_scene_feat_withobjects = torch.mul(object_attn_weights, attn_applied_scene_feat)

    #     scene_face_feat = torch.cat((attn_applied_scene_feat_withobjects, face_feat), 1)
    #     # attn_applied_scene_feat_withobjects = torch.mul(scene_face_feat, object_attn_weights)
    ...
    #     # encoding = self.compress_conv1(attn_applied_scene_feat_withobjects)
...
class ModelSpatioTemporal(nn.Module):
    ...
    def __init__(self, block=BottleneckConvLSTM, num_lstm_layers = 1, bidirectional = False, layers_scene = [3, 4, 6, 3, 2], layers_face = [3, 4, 6, 3, 2]):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.avgpool = nn.AdaptiveAvgPool2d(7, stride=1)
    ...
    def forward(self, images, head, face, hidden_scene: tuple = None, batch_sizes: list = None):
        ...
        # reduce head channel size by max pooling: (N, 1, 224, 224) -> (N, 1, 28, 28)
        head_reduced = self.maxpool(self.maxpool(self.maxpool(head))).view(-1, 65536)
        # print("head_reduced !!!!!!!!!", head_reduced.shape)
        # reduce face feature size by avg pooling: (N, 1024, 7, 7) -> (N, 1024, 1, 1)
        face_feat_reduced = self.avgpool(face_feat).view(-1, 1024)
        # print("face_feat_reduced !!!!!!!!!", face_feat_reduced.shape)
        # get and reshape attention weights such that it can be multiplied with scene feature map
        attn_weights = self.attn(torch.cat((head_reduced, face_feat_reduced), 1))
        attn_weights = attn_weights.view(-1, 1, 49)
        attn_weights = F.softmax(attn_weights, dim=2) # soft attention weights single-channel
