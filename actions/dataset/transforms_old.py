...
class Augmentation(object):
    ...
    def __call__(self, video_clip, target):
        ...
        print(np.array(video_clip[0]))
        # random flip
        flip = False #random.randint(0, 1)
        if flip:
        ...
        if target is not None:
            print(target)
            target = self.apply_bbox(target, ow, oh, 0, 0, 1, 1)
            '''
            if flip:
...
class BaseTransform(object):
    ...
    def __call__(self, video_clip, target=None, normalize=False):
        ...
        if target is not None:
            oh_new = video_clip[0].height
            ow_new = video_clip[0].width
        
            width_ratio = ow_new / ow
            height_ratio = oh_new / oh
        
            #print(width_ratio, height_ratio)
            target[..., 0] = target[..., 0]  * width_ratio
            target[..., 1] = target[..., 1]  * height_ratio
            target[..., 2] = target[..., 2]  * width_ratio
            target[..., 3] = target[..., 3]  * height_ratio
